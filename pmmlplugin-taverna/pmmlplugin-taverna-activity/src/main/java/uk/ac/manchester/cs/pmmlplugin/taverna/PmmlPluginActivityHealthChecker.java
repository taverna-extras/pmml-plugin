...
/**
 * PmmlPlugin health checker
 * 
 */
public class PmmlPluginActivityHealthChecker implements
		HealthChecker<PmmlPluginActivity> {
	...
	public boolean canVisit(Object o) {
		...
		return o instanceof PmmlPluginActivity;
	}
	...
	public VisitReport visit(PmmlPluginActivity activity, List<Object> ancestry) {
		PmmlPluginActivityConfigurationBean config = activity.getConfiguration();
		...
		if (config.getPmmlFile().equals("")) {
			// Warning on possible problems
			subReports.add(new VisitReport(HealthCheck.getInstance(), activity,
					"PMML document empty", HealthCheck.NO_CONFIGURATION,
					Status.WARNING));
		}
		...
		return new VisitReport(HealthCheck.getInstance(), activity,
				"PmmlPlugin service OK", HealthCheck.NO_PROBLEM, subReports);
	}
